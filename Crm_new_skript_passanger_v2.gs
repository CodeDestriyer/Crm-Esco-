...
// Знайти всі рядки де колонка = значення
...
// Масове оновлення поля для кількох пасажирів
...
// Масове призначення рейсу (CAL_ID) кільком пасажирам
function apiAssignTrip(params) {
  ...
  // Оновлюємо лічильник зайнятих місць в Календарі
  updateCalendarOccupancy(calId);

  return { ok: true, updated: updated };
}

// Оновити зайнятість рейсу після зміни
function updateCalendarOccupancy(calId) {
  ...
  // Порахувати скільки пасажирів мають цей CAL_ID
  var count = 0;
  [SHEETS.PAX_UE, SHEETS.PAX_EU].forEach(function(shName) {
    var sh = getSheet(shName);
    if (!sh) return;
    ...
    if (calIdx === -1) return;
    for (var i = 0; i < info.data.length; i++) {
      if (String(info.data[i][calIdx]) === String(calId)) count++;
    }
  });
  ...
  if (freeIdx !== -1) calSheet.getRange(found.rowNum, freeIdx + 1).setValue(Math.max(0, maxSeats - count));
}
...
function apiDeletePassenger(params) {
  ...
  // Якщо був призначений рейс — оновити лічильник
  var obj = rowToObj(found.headers, found.data);
...
function apiCreateTrip(params) {
  ...
  for (var v = 0; v < vehicles.length; v++) {
    ...
    // Generate free list (without driver seat)
...
function apiUpdateTrip(params) {
  ...
  if (params.auto_name) obj['Назва авто'] = params.auto_name;
...
// ════════════════════════════════════════════
// API — Passengers by Trip (для відображення хто в рейсі)
// ════════════════════════════════════════════
...
function doGet(e) {
  ...
  try {
    if (action === 'getAll') {
      ...
      result = { ok: true, message: 'CRM v2 API працює!', timestamp: new Date().toISOString() };
    }
  } catch (err) {
  ...
  try {
    switch (action) {
      ...
      case 'bulkUpdateField':
        result = apiBulkUpdateField(body);
        break;
      case 'assignTrip':
        result = apiAssignTrip(body);
        break;
      ...
      // Passengers by trip
      case 'getPassengersByTrip':
        result = apiGetPassengersByTrip(body);
        break;
